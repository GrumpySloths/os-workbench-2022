...
int parseOptions(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            if (!strcmp(argv[i], options[op].name) || !strcmp(argv[i], options[op].full_name))
            {
                hasMatch = true;
...
struct process *readProcess(char *pidStr, struct process *parent)
{
    ...
    if (sfp)
    { // process may die before this moment
        ...
        if (parent)
        {
            ...
            else
            {
                if (OP_NUMERIC)
                {
                    ...
                    {
                        while (child->next && proc->pid > child->next->pid)
                            child = child->next;
